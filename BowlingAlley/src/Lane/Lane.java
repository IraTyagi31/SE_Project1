...
public class Lane extends LaneUtil implements PinsetterObserver {
		...
		while (true) {
			if (partyAssigned && !gameFinished) {	// we have a party on this lane, so next bower can take a throw
				while (gameIsHalted) {
					try {
						sleep(10);
					} catch (Exception e) {System.err.println("Error: Unable to Halt Game. "+ e );}
				}
			...
			try {
				sleep(10);
			} catch (Exception e) {System.err.println("Error: While sleeping "+ e );}
		}
	}
	
	/*
	 * New function 
	 * Simulate ball throw from currentThrower
	 * After ball throw by a player is complete update ScoreHistoryFile 
	 */
		...
		if (bowlerIterator.hasNext()) {
			...
			if (frameNumber == 9){
				...
				try{
					...
					String dateString = String.format("%d:%d,%d/%d/%d ", cal.get(Calendar.HOUR), cal.get(Calendar.MINUTE), cal.get(Calendar.MONTH), cal.get(Calendar.DATE),  cal.get(Calendar.YEAR) + 1900);
					ScoreHistoryFile.addScore(currentThrower.getNick(), dateString, Integer.toString(cumulScores[bowlIndex][9]));
				} catch (Exception e) {System.err.println("Error: AddScore not working. "+ e );}
			}
	...
	private void markScore( Bowler Cur, int frame, int ball, int score ){
		...
		cumulScores[bowlIndex] = new ScoreUtil(curScore, index).getScores();
		publish( lanePublish() );
	}
	...
	/*
	 * New Function
	 * After completion of game ask user to continue another game 
	 * or stop and print result.
	 */
	...
	/*
	 * New Function 
	 * Function for getting game details after completion for storing and printing
	 * 
	 */
	public String getGameDetails(){
		Calendar cal = Calendar.getInstance(); //cal.get(Calendar.YEAR)
		String store = String.format("%d:%d,%d/%d/%d ", cal.get(Calendar.HOUR), cal.get(Calendar.MINUTE), cal.get(Calendar.MONTH), cal.get(Calendar.DATE),  cal.get(Calendar.YEAR) + 1900);

		Vector party_members = party.getMembers();

		int no_players = party_members.size();

		for(int i=0; i < no_players; i++){
			store += ((Bowler) party_members.get(i)).getNick()+" ";
		}
		...
		for(int i=0; i < no_players; i++){
			...
			int[] save_scores = (int[]) scores.get(party_members.get(i));
